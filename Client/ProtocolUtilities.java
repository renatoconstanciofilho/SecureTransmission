...
public class ProtocolUtilities {
	...
	public static void printByteArray(String msg, byte[] byteArray) { //para debug
	 ...
	 * @throws IOException
	 */
	public static ArrayList<String> quebraCabecalho(InputStream in) throws IOException {
		ArrayList<Character> pipeline = new ArrayList<>();
		StringBuilder cabecalho = new StringBuilder();
		int c;
		while ((c = in.read()) != -1) {
			pipeline.add((char) c);
			cabecalho.append((char) c);
			if (pipeline.size() != 2) // pipeline não está cheio
				continue;
			if (pipeline.get(0) == '\n' && pipeline.get(1) == '\n') {
				cabecalho.deleteCharAt(cabecalho.length()-1);
				break;
			}
			pipeline.remove(0); // mantem apenas 2 bytes mais recentes
		}
		if (cabecalho.length() == 0) return null;
		ArrayList<String> headerParts = new ArrayList<String>();
		Scanner scanner = new Scanner(cabecalho.toString());
